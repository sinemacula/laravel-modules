...
```bash
git clone git@github.com:sinemacula/laravel-modules.git
cd laravel-modules
composer install
```
...
```text
feat: add stub customization to module:make
fix: preserve module order during cache rebuild
test: cover the resourcePath namespace fallback
...
```bash
vendor/bin/phpunit tests/Unit/Configuration/ModulesTest.php
vendor/bin/phpunit --filter testModulesPathReturnsBasePathPlusModules tests/Unit/Configuration/ModulesTest.php
```
